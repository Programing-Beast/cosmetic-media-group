...
export default async function ServicesPage() {
  ...
  return <div className="page-enter"><PageHero title="Integrated expertise." accent="One clear objective." intro="Reputation, profile, production, training, audio and experiences brought together around the outcome that matters most." crumbs={[{label: 'Home', href: '/'}, {label: 'Services'}]} /><section><div className="shell"><div className="service-overview-grid">{services.map((service, index) => <Link className="service-tile" href={`/services/${service.slug}`} key={service.slug}><div><div className="service-tags">{String(index + 1).padStart(2, '0')} / {service.eyebrow}</div><h3>{service.title}</h3><p>{service.intro}</p></div><div className="story-image" style={{height: 150}}><Image src={imageUrl(service.image, 700, 360)} alt={imageAlt(service.image, service.title)} width={700} height={360} sizes="(max-width: 980px) 100vw, 33vw" /></div><b>Explore the service ↗</b></Link>)}</div></div></section><section className="services-editorial-closing"><div className="shell"><h2>One idea. Every relevant channel.</h2><div><p>The strongest work rarely sits inside a single service. We connect communications, editorial thinking, production, events and expert positioning around one clear commercial and reputational objective.</p><Link className="btn btn-white" href="/contact">Discuss your brief ↗</Link></div></div></section><NewsletterBand /></div>
}
