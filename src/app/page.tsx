...
export default async function HomePage() {
  ...
  return <div className="page-enter editorial-home-v2"><section className="cmg-video-hero cmg-video-hero-full"><video className="cmg-video-hero-film" src="/videos/cmg-hero-film.mp4" muted autoPlay loop playsInline preload="metadata" /></section><section className="cmg-video-hero"><div className="shell"><video className="cmg-video-hero-film" src="/videos/cmg-hero-film.mp4" muted autoPlay loop playsInline preload="metadata" /></div></section><section className="cmg-cover"><div className="shell"><div className="cmg-cover-grid"><div className="cmg-cover-copy"><div><div className="cmg-issue-line"><span>{hero.eyebrowLeft}</span><span>{hero.eyebrowRight}</span></div><h1>{hero.titleBefore} <em>{hero.titleAccent}</em></h1></div><div className="cmg-cover-deck"><div><p className="cmg-identity-line">{hero.identity}</p><p className="cmg-cover-summary">{hero.summary}</p></div><div className="cmg-cover-actions"><Link className="btn btn-dark" href={hero.primaryCta.href}><ArrowLabel text={hero.primaryCta.label} /></Link><Link className="btn btn-pink" href={hero.secondaryCta.href}><ArrowLabel text={hero.secondaryCta.label} /></Link></div></div></div><Link className={`cmg-cover-image ${hero.videoUrl || hero.film?.length ? 'cmg-cover-film' : ''}`} href="/media-hub" aria-label="Editorial film concept">{hero.videoUrl ? (
