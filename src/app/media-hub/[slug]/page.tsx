...
export async function generateMetadata({params}: {params: Promise<{slug: string}>}) {
  ...
  if (!video) return {}
  return createMetadata(video.title, video.intro || `${video.title} — video from Cosmetic Media Group.`, video.seo, `/media-hub/${slug}`)
}
  ...
  return (
    <div className="page-enter">
      <div className="video-hero">
        <div className="shell">
          <div className="crumbs"><Link href="/">Home</Link><span>/</span><Link href="/media-hub">Media Hub</Link><span>/</span><span>{video.title}</span></div>
          <h1>{video.title}</h1>
      ...
      <section>
        <div className="shell video-detail">
          ...
          <div className="video-back"><Link className="text-link" href="/media-hub">← Back to Media Hub</Link></div>
        </div>
      </section>
    </div>
  )
