...
export function HomeSections({homepage, services, articles, brands, toolkits}: {homepage: Homepage; services: Service[]; articles: Article[]; brands: Brand[]; toolkits: Toolkit[]}) {
  const renderers: Record<HomeSectionType, React.ReactNode> = {
    ...
    brands: <BrandsSection key="brands" homepage={homepage} brands={brands} />,
    resourcesMembership: <ResourcesMembership key="resourcesMembership" homepage={homepage} toolkits={toolkits} />,
...
function MediaHubSection({homepage, articles}: {homepage: Homepage; articles: Article[]}) {
  ...
  return <section className="cmg-media"><div className="shell"><div className="cmg-media-title"><div><div className="cmg-section-no">04 — Latest from Cosmetic Media Group</div><p>A dynamic mix of editorial, video, PR insight, interviews, industry news, podcasts and behind-the-scenes content.</p></div><h2>The stories, voices and ideas shaping aesthetics now.</h2></div><LatestLayout><Link className="cmg-feature-story" href={`/media-hub/${featured.slug}`}><Image src={imageUrl(featured.image, 900, 700)} alt={imageAlt(featured.image, featured.title)} width={900} height={700} sizes="(max-width: 980px) 100vw, 55vw" /><div className="cmg-feature-copy"><div><small>Latest {featured.format || 'article'} / {featured.category}</small><h3>{featured.title}.</h3><p>{featured.excerpt}</p></div><b>Read the feature <Arrow /></b></div></Link><div className="cmg-side-stories">{side.map((article) => <Link className="cmg-side-story" href={`/media-hub/${article.slug}`} key={article.slug}><Image src={imageUrl(article.image, 700, 420)} alt={imageAlt(article.image, article.title)} width={700} height={420} sizes="(max-width: 980px) 100vw, 45vw" /><div><small>{article.format === 'video' ? 'Latest video' : article.format === 'interview' ? 'Featured interview' : article.format || article.category}</small><h4>{article.title}.</h4></div></Link>)}</div></LatestLayout>{rail.length > 0 && <div className="cmg-latest-rail">{rail.map((item) => <Link className="cmg-latest-item" href={item.href || '/media-hub'} key={item.title}><small>{item.label}</small><h4>{item.title}</h4><span><ArrowLabel text={item.actionLabel || 'Read more ↗'} /></span></Link>)}</div>}<div className="cmg-media-categories">{[
