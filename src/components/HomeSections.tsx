...
function BrandsSection({homepage, brands}: {homepage: Homepage; brands: Brand[]}) {
  ...
  return <section className="cmg-brands"><div className="shell"><div className="cmg-brands-head"><div><div className="cmg-section-no">08 — Our brands</div><h2>Distinct platforms. One clear industry purpose.</h2></div><Link className="text-link" href="/our-brands">Explore the ecosystem ↗</Link></div><div className="cmg-brand-list">{rows.map((row, index) => <Link className="cmg-brand-line" href={row.href || '/our-brands'} key={row.title}><span>{String(index + 1).padStart(2, '0')} / {row.label}</span><h3>{row.title}</h3><p>{row.description}</p><b>↗</b></Link>)}</div></div></section>
}
