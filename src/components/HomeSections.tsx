...
function MediaHubSection({articles}: {articles: Article[]}) {
  ...
  return <section className="cmg-media"><div className="shell"><div className="cmg-media-title"><div><div className="cmg-section-no">04 — Latest from the Media Hub</div><p>Articles, interviews, videos, podcasts, news, trends, opinions and expert intelligence.</p></div><h2>Discover the people and ideas moving aesthetics forward.</h2></div><div className="cmg-media-layout"><Link className="cmg-feature-story" href={`/media-hub/${featured.slug}`}><img src={imageUrl(featured.image, 900, 700)} alt={imageAlt(featured.image, featured.title)} /><div className="cmg-feature-copy"><div><small>Latest {featured.format || 'article'} / {featured.category}</small><h3>{featured.title}.</h3><p>{featured.excerpt}</p></div><b>Read the feature ↗</b></div></Link><div className="cmg-side-stories">{side.map((article) => <Link className="cmg-side-story" href={`/media-hub/${article.slug}`} key={article.slug}><img src={imageUrl(article.image, 700, 420)} alt={imageAlt(article.image, article.title)} /><div><small>{article.format || article.category}</small><h4>{article.title}</h4></div></Link>)}</div></div><div className="cmg-media-categories">{['All stories', 'Articles', 'Interviews', 'Videos', 'Podcasts', 'Industry news', 'Opinion', 'Expert content'].map((item) => <Link href="/media-hub" key={item}>{item}</Link>)}</div></div></section>
}
