    ...
    <footer>
      <div className="shell">
        <div className="footer-grid">
          ...
          <div className="footer-col"><h4>Resources</h4><Link href="/media-desk">Media Desk for Journalists</Link><Link href="/toolkits">Toolkits</Link><Link href="/media-archive">Articles & interviews</Link><Link href="/toolkits">Research & reports</Link><Link href="/contact">Contact</Link>{settings.socialLinks?.map((link) => <a key={link.platform} href={link.url}>{link.platform}</a>)}</div>
        </div>
